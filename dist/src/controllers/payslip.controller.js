...
class PayslipController {
    ...
    static async getPayslipsByPayrun(req, res) {
        try {
            ...
            const pagination = (0, pagination_utils_1.getPaginationParams)(req.query);
            if (!payrunId)
    ...
    static async getAllPayslips(req, res) {
        try {
            ...
            const result = await payslipService.getAllPayslipsPaginated(entrepriseId, Number(page), Number(limit));
            res.json(result);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : errors_messages_1.ERROR_MESSAGES.ERROR_SURVENUE;
