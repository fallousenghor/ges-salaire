...
export function generateRecuPDF(res: Response, paiement: any, employe: any, entreprise: any) {
  ...
  doc.fontSize(8)
     .fillColor('#888')
     .text(`Document généré le ${new Date().toLocaleDateString('fr-FR')} par ${entreprise.nom}`,
           40, bottomY, { align: 'center' });

  doc.end();
}

// Génère un bulletin de paie PDF avec design professionnel
export function generatePayslipPDF(res: Response, payslip: any, employe: any, entreprise: any) {
  ...
  doc.font('Helvetica-Bold')
     ...
     .fillColor(entreprise.couleurPrimaire || '#2563eb')
     .text(formatMontant(payslip.netAPayer) + ' ' + (entreprise.devise || 'FCFA'),
           420, totalY + 10, { width: 100, align: 'right' });

  // Pied de page
  const bottomY = doc.page.height - 50;
  doc.fontSize(8)
     .fillColor('#666')
     .text(`Document généré le ${new Date().toLocaleDateString('fr-FR')} - ${entreprise.nom}`,
